...
- **模块划分**：`modules/vocabulary/` 与 `modules/corpus/` 平级，采用 **core / ui** 分层（对齐 [ai/code-style/microbleed/README.md](ai/code-style/microbleed/README.md)）：
  - **core/**：领域逻辑、与框架无关。含 `types.ts`（接口 I\* 命名）、`constants.ts`、`ai-schema.ts`、`ai-config.ts`、`vocabulary.ts`（IVocabulary 与列表/筛选状态形态）。Server Actions 在 `modules/vocabulary/actions/`（filter、crud、import、ai 等）。
...
| Server Actions | `modules/vocabulary/actions/`           | CRUD 单词、getVocabularyEntries、getVocabularyFilterOptions、AI 补全/批量解析                                                                                                                                                                                                                    |

/**
 * 从词汇表图片中提取 word / phonetic / meaning 等为 JSON。
 * 使用 OpenRouter 视觉模型（通过 openai 包 + baseURL）与 Zod 结构化输出。
 *
 * 用法:
 *   pnpm tsx scripts/extract-words-from-image.ts <图片路径|目录> [--output <文件路径>]
...
const wordItemSchema = z.object({
  ...
  exampleEn: z.string().describe("例句英文"),
  exampleCn: z.string().describe("例句中文"),
  collocations: z
    .array(
      z.object({
        collocation: z.string().describe("固定搭配短语"),
        meaning: z.string().describe("中文释义"),
      }),
    )
    .describe("固定搭配"),
  notes: z
    .array(
      z.object({
        word: z.string().describe("单词"),
        phonetic: z.string().describe("音标(你需要补充)"),
        meaning: z.string().describe("中文释义"),
      }),
    )
    .describe("与该单词相关的其他单词"),
  remember: z.string().describe("记忆该单词的方法"),
});

/** 结构化输出要求根为 object，故用 items 包装数组 */
const vocabListResponseSchema = z.object({
  items: z.array(wordItemSchema).describe("从图片中识别出的词汇列表"),
});
...
function parseArgs(): ParsedArgs {
  ...
  if (!inputPath) {
    console.error(
      "用法: pnpm tsx scripts/extract-words-from-image.ts <图片路径|目录> [--output <文件路径>]\n" +
